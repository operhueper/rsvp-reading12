<script>
  ...
  async function handleFileSelect(event) {
    ...
    try {
      ...
      stop(); parseText(); showTextInput = false; stopDemo();
  ...
  async function openBook(bookId) {
    ...
    if (!book) return;
    text = book.text; parseText(); stopDemo();
  ...
  function startDemo() { demoIndex = 0; demoWord = demoWords[0]; demoInterval = setInterval(() => { demoIndex = (demoIndex + 1) % demoWords.length; demoWord = demoWords[demoIndex]; }, 400); }
  function stopDemo() { if (demoInterval) { clearInterval(demoInterval); demoInterval = null; } }

  function handleKeydown(e) {
    if (e.target.tagName === 'TEXTAREA' || e.target.tagName === 'INPUT') return;
    switch (e.code) {
      case 'Space': e.preventDefault(); if (isPlaying) pause(); else if (isPaused) resume(); else if (words.length > 0) start(); break;
      case 'Escape':
  ...
  onDestroy(() => { if (intervalId) clearTimeout(intervalId); if (fadeTimeoutId) clearTimeout(fadeTimeoutId); stopAutoSave(); stopDemo(); window.removeEventListener('keydown', handleKeydown); });
</script>
...
<main class:focus-mode={isFocusMode}>
  ...
  {#if showTextInput && !isFocusMode}<div class="panel-overlay" on:click|self={() => showTextInput = false} role="presentation"><TextInput {text} isLoading={isLoadingFile} {loadingMessage} on:apply={handleTextApply} on:fileselect={handleFileSelect} on:close={() => showTextInput = false} /></div>{/if}
  {#if showSettings && !isFocusMode}<div class="panel-overlay" on:click|self={() => showSettings = false} role="presentation"><Settings bind:wordsPerMinute bind:fadeEnabled bind:fadeDuration bind:pauseOnPunctuation bind:punctuationPauseMultiplier bind:wordLengthWPMMultiplier bind:pauseAfterWords bind:pauseDuration bind:frameWordCount on:close={() => showSettings = false} /></div>{/if}
    ...
    <div class="bottom-bar" class:minimal={isFocusMode}>
      ...
      {#if !isFocusMode}
        ...
        <div class="touch-controls mobile-only">
          ...
          <button class="touch-btn" on:click={() => currentWordIndex = Math.min(words.length, currentWordIndex + 10)}><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/></svg></button>
        </div>
      {/if}
    </div>
  {/if}
...
<style>
  ...
  /* WELCOME */
  .welcome { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; overflow-y: auto; -webkit-overflow-scrolling: touch; padding: 1rem 0; gap: 0; }
  .demo-container { position: relative; margin-bottom: 1.5rem; height: 4rem; display: flex; align-items: center; justify-content: center; }
  .demo-marker { position: absolute; left: 50%; transform: translateX(-50%); width: 2px; height: 100%; }
  .demo-marker::before, .demo-marker::after { content: ''; position: absolute; left: 0; width: 100%; height: 12px; background: #ff4444; }
  .demo-marker::before { top: 0; mask-image: linear-gradient(to bottom, #000, transparent); -webkit-mask-image: linear-gradient(to bottom, #000, transparent); }
  .demo-marker::after { bottom: 0; mask-image: linear-gradient(to top, #000, transparent); -webkit-mask-image: linear-gradient(to top, #000, transparent); }
  .demo-word { font-family: 'SF Mono', 'Monaco', 'Consolas', monospace; font-size: clamp(2.5rem, 8vw, 4rem); font-weight: 600; color: #fff; letter-spacing: 0.02em; }
  .welcome h2 { font-size: clamp(1.5rem, 5vw, 2.2rem); font-weight: 700; margin: 0 0 0.5rem; color: #fff; letter-spacing: -0.02em; }
  .welcome-sub { color: #777; font-size: clamp(0.85rem, 2.5vw, 1rem); max-width: 420px; line-height: 1.5; margin: 0 0 1.5rem; }
  .cta-btn { display: flex; align-items: center; gap: 0.6rem; padding: 0.9rem 2rem; background: #ff4444; border: none; border-radius: 12px; color: #fff; font-size: 1.1rem; font-weight: 600; cursor: pointer; transition: all 0.2s; }
  .cta-btn:hover { background: #ff5555; transform: translateY(-1px); }
  .cta-btn:active { transform: scale(0.97); }
  .cta-btn svg { width: 22px; height: 22px; }
  .formats-hint { color: #555; font-size: 0.8rem; margin: 0.5rem 0 0; }
  .continue-reading { width: 100%; max-width: 400px; margin-top: 1.5rem; }
  .continue-reading h4 { color: #666; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08em; margin: 0 0 0.5rem; }
  .continue-btn { width: 100%; background: #111; border: 1px solid #222; border-radius: 10px; padding: 0.75rem 1rem; cursor: pointer; text-align: left; color: #fff; margin-bottom: 0.4rem; transition: all 0.15s; }
  .continue-btn:hover { background: #1a1a1a; border-color: #333; }
  .continue-info { display: flex; justify-content: space-between; margin-bottom: 0.4rem; }
  .continue-title { font-size: 0.9rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 65%; }
  .continue-meta { color: #666; font-size: 0.75rem; font-family: monospace; }
  .continue-bar { height: 3px; background: #222; border-radius: 2px; overflow: hidden; }
  .continue-fill { height: 100%; background: #ff4444; }
  .features { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; width: 100%; max-width: 440px; margin-top: 1.5rem; }
  .feature { display: flex; gap: 0.6rem; text-align: left; padding: 0.6rem; background: #0a0a0a; border-radius: 10px; border: 1px solid #1a1a1a; }
  .feature-icon { font-size: 1.3rem; flex-shrink: 0; width: 2rem; text-align: center; }
  .feature-text { display: flex; flex-direction: column; }
  .feature-text strong { color: #ddd; font-size: 0.8rem; font-weight: 600; }
  .feature-text span { color: #555; font-size: 0.7rem; line-height: 1.3; }
  .how-to { margin-top: 1.25rem; width: 100%; max-width: 400px; }
  .how-to h4 { color: #666; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08em; margin: 0 0 0.6rem; }
  .steps { display: flex; gap: 0.5rem; justify-content: center; }
  .step { display: flex; align-items: center; gap: 0.4rem; color: #666; font-size: 0.8rem; }
  .step-num { width: 20px; height: 20px; background: #1a1a1a; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; color: #ff4444; font-weight: 700; flex-shrink: 0; }
  .step kbd { background: #1a1a1a; padding: 0.1rem 0.3rem; border-radius: 3px; font-family: monospace; font-size: 0.7rem; color: #888; }
